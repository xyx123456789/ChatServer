...
bool Redis::connect()
{
    ...
    if(publish_context_ == nullptr)
    {
        cerr<<"redis connect publish_context_ failed!"<<endl;
        return false;
    }
    ...
    if(subcribe_context_ == nullptr)
    {
        cerr<<"redis connect subcribe_context_ failed!"<<endl;
        return false;
    }

    //在单独的线程中，监听通道的事件，有消息给业务层上报
    thread t([&](){
...
bool Redis::subscribe(int channel)
{
    ...
    if(redisAppendCommand(this->subcribe_context_,"SUBSCRIBE %d",channel) == REDIS_ERR)
    {
    ...
    while(!done)
    {
        if(redisBufferWrite(this->subcribe_context_,&done) == REDIS_ERR)
        {
            cerr<<"redis subscribe command failed!"<<endl;
            return false;
        }
...
void Redis::observer_channel_message()
{
    ...
    while(redisGetReply(this->subcribe_context_,(void **)&reply) == REDIS_OK)
    {
        //订阅收到的消息是一个三元数组
